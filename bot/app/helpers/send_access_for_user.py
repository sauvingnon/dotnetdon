...
async def send_access_for_user(callback: CallbackQuery, state: FSMContext):
    ...
    callback.message.answer('Твой ключ ниже, далее:' \
    '1. Нажми на него, он скопируется в буфер обмена.' \
    '2. Зайди в приложение, нажми плюсик и вставь содержимое буфера обмена' \
    '3. Подключайся и наслаждайся.')
    ...
    await callback.message.answer(f'`{key_content}`', parse_mode='Markdown')
